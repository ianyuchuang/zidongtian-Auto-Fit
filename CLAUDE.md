...
- 網頁（純前端，照片只在瀏覽器本地處理）：`web/`，無建置步驟；推到 main 會自動發布到 GitHub Pages，網址與設定見 `docs/部署.md`。
  - `js/app.js` 狀態與動作；`js/ui/` 入口頁 / 頂列 / 左樹 / 表格 / 檢視器 / 燈箱（`dnd.js` 為表格與左樹共用的拖曳格式與提示，含多張拖曳的縮圖堆動畫；`recognize.js` 為頂列「AI 辨識」的設定對話框與進度）；`js/fs/` 資料夾存取（File System Access API，`memory.js` 為唯讀複本，`handle-store.js` 用 IndexedDB 記住上次的資料夾）；`js/recognizer/` 辨識模組（`mock`；`local.js` + `local/` 為本機 llama-server，照片不出這台電腦，見 `docs/本地模型.md`；`api.js` + `api/` 為 LLM API 直打，四家定義與申請方式見 `docs/LLM-API.md`）；`js/template/` 版型（`spec.js` LayoutSpec 與純邏輯、`parse.js` 解析 docx、`parse-xlsx.js` 解析 xlsx、`fields.js` 欄位名判斷兩邊共用、`xlsx.js` Excel 單位換算、`xml.js`、`unzip.js`、`library.js` 版型庫、`builtin.js` 讀 `web/templates/` 的內建版型），見 `docs/版型.md`；`js/docx-export.js` 照 LayoutSpec 產 Word（`vendor/docx-*.iife.js`）、`js/xlsx-export.js` 照**同一份** LayoutSpec 產 Excel（`js/zip.js` 用瀏覽器內建 `CompressionStream` 自己打包 ZIP，不外掛 Excel 函式庫）、`docx-model.js` 管分組、檔名、格式（`defaultFormat`）與輸出前檢查；`js/pdf-export.js` 是「同時產出 PDF」——照同一份 LayoutSpec **自己畫**（`vendor/pdf-lib-*.min.js` + `vendor/fontkit-*.umd.min.js`），把這次每一份 Word 的每一頁接成一份「根資料夾名.pdf」放根資料夾。瀏覽器沒辦法真的轉檔 docx→PDF，所以是重畫，行高／字距會有極細微差異。字型見 `js/pdf-font.js`：先用 `queryLocalFonts()` 拿這台電腦的標楷體（要在使用者點擊事件裡呼叫才跳得出授權，所以是勾選框一打勾就先要），拿不到才退回 `vendor/fonts/TW-Kai-98_1.ttf`（全字庫正楷體，政府資料開放授權；另外兩份 TW-Kai 不進版控，見 .gitignore）。嵌入時會子集化，PDF 不會變肥。
...
- 工具套件清單 `tools/requirements-tools.txt`（pytest、pillow、Google API）；缺的時候 `tools/deps.py` 會問要不要現在裝。
  ...
  - 版型：入口頁按「讀取版型」進**版型頁**（`ui/template-page.js`）——先「選版型」（最頂端是 `web/templates/` 的**內建版型**——由 `tools/make_template_fixtures.py` 從實際自檢表抽出並匿名，同一批檔也是解析器的測試樣本，版面相同的只列一份；接著是存過的版型／預設版面／讀一份新的 docx 或 xlsx），讀了檔才進「版型調整」，整頁所見即所得地調（抬頭與說明格直接點著打字——點格子空白處也接得到游標，Enter 分段、Shift+Enter 段落內換行；要填值的位置是可換欄位的「膠囊」，同一行可以放好幾個、中間夾自己打的逗號；欄位可從工具列拖進某一行或新增一行、也能拖到別格；拖曳格子換填照順序；格子上按右鍵可選水平／垂直對齊），「完成」會先存進版型庫（`autofit:v1:templates`）再回入口頁，並記住每個資料夾上次用哪一份。一次讀取只用一種版型。細節與五份樣本的差異見 `docs/版型.md`。
  - 輸出格式：Word 或 Excel，在「產生檔案」對話框選（預設跟版型的來源一樣：xlsx 版型 → Excel）。兩種吃同一份 LayoutSpec，Excel 版的差異（抬頭每頁一列、`rowBreaks` 分頁、照片是浮動圖片、列高取大值）見 `docs/版型.md`。
